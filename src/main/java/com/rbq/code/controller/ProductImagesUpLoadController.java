...
@Controller
@Slf4j
public class ProductImagesUpLoadController {
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/product/uploadFile", method = RequestMethod.POST)
    public String uploadFile(HttpServletRequest servletRequest, @RequestParam("file") MultipartFile file)
        ...
        if (!file.isEmpty()) {
            ...
            // 返回的是一个url对象,图片名称
            String  demo ="/images/product/"+filename;
            return demo;
        } else {
            return null;
        }
    ...
    @ResponseBody
    @RequestMapping(value = "/product/imageload", method = RequestMethod.POST)
    public Map<String, Object> test(HttpServletRequest servletRequest, @RequestParam("file") MultipartFile file)
            throws IOException {
        // 如果文件内容不为空，则写入上传路径
        if (!file.isEmpty()) {
            // 上传文件路径
            ...
            // 返回的是一个url对象,图片名称
            res.put("url", "/images/product/" + filename);
            return res;
        } else {
            return null;
        }
